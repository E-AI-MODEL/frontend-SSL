      ...
      <div className="flex-1 flex flex-col relative overflow-hidden bg-white">
        <div className="h-14 flex items-center px-8 border-b border-[#141414] bg-[#D8D7D4] justify-between shrink-0">
          <h2 className="text-xl font-serif italic text-[#141414] flex items-center gap-2">
            ...
            {activeTab === 'overview' && 'Data Interpretation'}
            {activeTab === 'seeds' && 'Validation Gate & Seed Review'}
            {activeTab === 'papers' && 'Adversarial Paper Analysis'}
            {activeTab === 'probes' && 'Probe Generation Engine'}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#E4E3E0] relative">
            ...
            {runningBenchmark && (
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto space-y-1">
                  <div className="opacity-50 mb-2">$ npm run test:blind -- --env Node24</div>
                  ...
                  {benchmarkStatus === 'completed' && (
                    <div className="mt-4 text-[#0066CC] font-bold">» Benchmark finished successfully. Metrics pushed to artifact cache.</div>
                  )}
                </div>
              </motion.div>
            )}
          ...
          {loading || !summary ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === 'overview' && <OverviewView summary={summary} />}
                {activeTab === 'seeds' && <SeedAnalysisView summary={summary} />}
                {activeTab === 'papers' && <PaperPipelineView summary={summary} />}
                {activeTab === 'probes' && <ProbeView summary={summary} />}
              </motion.div>
            </AnimatePresence>
          )}
        </div>
...
// -- OVERVIEW TAB --
function OverviewView({ summary }: { summary: Summary }) {
  const chartData = [
    { name: 'Active', value: summary.active_gaps, color: '#0066CC' },
    { name: 'Promoted', value: summary.promoted_seeds, color: '#00FF00' },
    { name: 'Dormant', value: summary.dormant_seeds, color: '#71717a' },
  ];

  return (
    <div className="space-y-6 pb-12">
      <div className="mb-6 border-b border-[#141414] pb-6">
        <h1 className="font-serif italic text-3xl mb-2">Research Intelligence Layer</h1>
        <p className="text-[#141414] opacity-70 font-mono text-[10px] uppercase tracking-widest max-w-2xl leading-relaxed">
          Translating structural absences and gaps identified in SSL 4.5. Monitor regressional stability and Phase 3 validation tracking metrics.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard title="Total Seeds" value={summary.total_seeds} />
        <StatCard title="Promoted (Weight ≥ 1.0)" value={summary.promoted_seeds} highlight="green" />
        <StatCard title="Active Probes" value={summary.active_gaps} highlight="blue" />
        <StatCard title="Dormant Tracks" value={summary.dormant_seeds} highlight="zinc" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        <div className="bg-[#EBEAE7] border border-[#141414] p-6 shadow-[2px_2px_0px_#141414]">
          <h3 className="font-serif italic text-xs uppercase mb-4 opacity-60">Seed Distribution</h3>
          <div className="h-64">
        ...
        <div className="bg-[#EBEAE7] border border-[#141414] p-6 flex flex-col shadow-[2px_2px_0px_#141414]">
          <h3 className="font-serif italic text-xs uppercase mb-4 opacity-60">Phase Progress</h3>
...
// -- SEEDS TAB --
function SeedAnalysisView({ summary }: { summary: Summary }) {
  ...
  const analyze = async (seed: Seed) => {
    ...
    setAnalysisText('');
    try {
      const result = await AIService.analyzeSeed(seed);
  ...
  return (
    <div className="space-y-6 h-full flex flex-col pb-12">
      <div className="border-b border-[#141414] pb-4">
        <h2 className="font-serif italic text-2xl mb-1">Validation Gate</h2>
        <p className="font-mono text-[10px] opacity-60 uppercase tracking-widest">Review trace/weight states. Select a seed to simulate the AI validation logic.</p>
      </div>
        ...
        <div className="bg-white border border-[#141414] shadow-[2px_2px_0px_#141414] p-6 flex flex-col relative overflow-hidden">
          {!selectedSeed ? (
            <div className="flex-1 flex flex-col items-center justify-center opacity-50 text-center p-8">
              ...
              <p className="font-mono text-[10px] uppercase tracking-widest">Select a seed from the Validation Gate</p>
            </div>
          ) : (
            <>
              <div className="mb-6 border-b border-[#141414] pb-4">
                ...
                <div className="text-[#141414] text-xs leading-relaxed mb-4 border-l-4 border-[#141414] pl-4 font-mono bg-[#FFF9E6] py-3 pr-3">
                  <span className="font-bold opacity-50 text-[10px] uppercase tracking-widest block mb-1">Gap Description:</span>
                  {selectedSeed.gap_description}
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="border border-[#141414] p-3 flex flex-col">
                    <span className="text-[10px] font-bold opacity-40 uppercase tracking-widest">Trace</span>
                  ...
                  <div className="border border-[#141414] p-3 flex flex-col bg-[#FFF9E6]">
                    <span className="text-[10px] font-bold opacity-40 uppercase tracking-widest">Weight</span>
              ...
              <div className="flex-1 overflow-y-auto">
                {analyzing ? (
                  <div className="flex items-center gap-3 text-[#141414] font-mono text-[10px] uppercase font-bold tracking-widest">
                    ...
                    Analyzing Validation Gate logic via Gemini...
                  </div>
                ) : (
...
// -- PAPERS TAB --
function PaperPipelineView({ summary }: { summary: Summary }) {
  ...
  const handleAnalyze = async () => {
    ...
    setAnalysis('');
    try {
      const result = await AIService.analyzePaper(text, summary.seeds);
      setAnalysis(result);
    } catch(e) {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col pb-12">
       <div className="border-b border-[#141414] pb-4">
        <h2 className="font-serif italic text-2xl mb-1">Paper Pipeline</h2>
        <p className="font-mono text-[10px] opacity-60 uppercase tracking-widest">Identify structural absences in academic literature and map to SSL gaps.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-[500px]">
        <div className="flex flex-col bg-white border border-[#141414] p-4 shadow-[2px_2px_0px_#141414]">
          <div className="mb-4">
            ...
            >
              <option value="manual">-- Manual Upload / Paste Text --</option>
              {MOCK_PAPERS.map(p => (
          ...
          <textarea
            ...
            placeholder="Paste paper abstract or content here..."
        ...
        <div className="bg-white border border-[#141414] shadow-[2px_2px_0px_#141414] p-6 overflow-y-auto">
          {analysis ? (
            ...
            <div className="h-full flex flex-col items-center justify-center opacity-50 text-center">
              ...
              <p className="font-mono text-[10px] uppercase tracking-widest max-w-[200px]">Paste text and click 'Run' to let the Intelligence Layer map it against {summary.total_seeds} existing seeds.</p>
            </div>
          )}
        </div>
      </div>
...
// -- PROBES TAB --
function ProbeView({ summary }: { summary: Summary }) {
  ...
  const generate = async () => {
    ...
    setProbes('');
    try {
      const result = await AIService.generateProbes(summary.seeds);
      setProbes(result);
    } catch(e) {
  ...
  return (
    <div className="space-y-6 max-w-4xl pb-12">
      <div className="border-b border-[#141414] pb-4">
        <h2 className="font-serif italic text-2xl mb-1">Dialectical Probes Generator</h2>
        <p className="font-mono text-[10px] opacity-60 uppercase tracking-widest">Generate adversarial follow-ups based on Promoted seeds (run-probe-utility-benchmark).</p>
      </div>
      ...
      <div className="bg-white border border-[#141414] shadow-[2px_2px_0px_#141414]">
        <div className="flex items-center justify-between p-6 border-b border-[#141414] bg-[#EBEAE7]">
          <div>
            <div className="text-xl font-bold font-mono text-green-600 mb-1">{promotedCount} PROMOTED SEEDS AVAILABLE</div>
            <div className="text-[10px] font-bold opacity-50 uppercase tracking-widest">Weights ≥ 1.0. Ready for adversarial test generation.</div>
          </div>
          <button
              onClick={generate}
          ...
          {generating ? (
            ...
            <div className="font-mono text-[11px] leading-relaxed prose prose-sm max-w-none text-[#141414] bg-white p-6 border border-[#141414] shadow-[2px_2px_0px_#141414] marker:text-[#141414] prose-h3:font-serif prose-h3:italic prose-h3:text-lg">
             ...
             <div className="flex flex-col items-center justify-center h-[200px] opacity-50">
               ...
               <p className="font-mono text-[10px] uppercase tracking-widest">Hit generate to create targeted adversarial evaluations for Phase 3 review.</p>
             </div>
          )}
        </div>
      </div>
